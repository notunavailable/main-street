...
const Outside = ({ shops, characterInput, setInside, ...props }) => {
    ...
    useEffect(() => {
        ...
        shops.forEach(shop => {
            let map = new Image()
            map.src = shop.outside.path
            mapPieces.push(map)
            pieceWidth = map.width

            let foreground = new Image()
            foreground.src = shop.outside.foreground
            foregroundPieces.push(foreground);

            collisionPieces.push(shop.outside.collisions)
            collisionTransportPieces.push(shop.outside.collisionTransport);
                ...
                row.forEach((symbol, columnIndex) => {
                    if (symbol === 67) {
                        boundaries.push(new Boundary({
                            position: {
                                x: (pieceIndex * pieceWidth) + (columnIndex * Boundary.width) + offset.x,
        ...
        collisionsTransportMapPieces.forEach((collisionTransportsMap, pieceIndex) => {
            collisionTransportsMap.forEach((row, rowIndex) => {
                row.forEach((symbol, columnIndex) => {
                    if (symbol === 67) {
                        transports.push({transport: new Boundary({
                            position: {
                                x: (pieceIndex * pieceWidth) + (columnIndex * Boundary.width) + offset.x,
                                y: (rowIndex * Boundary.height) + offset.y
                            },
            ...
            if (keys.w.pressed && lastKey === 'w') {
                ...
                for (let i = 0; i < transports.length; i++) {
                    ...
                    if (rectangularCollision({
                        ...
                        window.location = `http://www.localhost:3000/${transport.number}`
